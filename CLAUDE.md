...
**~375 TS/TSX files · 60+ pages · 56 API hooks · 41 UI components · 12 Zustand stores · 67+ E2E specs · 1244/1244 Vitest · Version 3.1.3** (Niveles 1-7 CLOSED — ROADMAP COMPLETE 🎉). Post-roadmap: Session/Auth Overhaul **W1+W2** ✅ SHIPPED 2026-06-05 (agent-aware idle-timeout + refresh-cookie fix; new `src/core/session/*`; PR #75 merged) — see [ADR-0009](docs/decisions/0009-agent-presence-session-work-continuity.md). Session/Auth **W3** (server-side agent liveness / anti-zombie: heartbeat + TTL + leader-gated reaper + `pagehide` departure beacon; new `src/core/presence/*`) ✅ SHIPPED 2026-06-06 — see [ADR-0009](docs/decisions/0009-agent-presence-session-work-continuity.md) + [W3 spec](docs/specs/2026-06-06-w3-agent-liveness.md). Session/Auth **W4** (deferred pause / pause-when-free: agent `PendingState` blocks new work immediately but the visible state only flips once active work drains, via a leader-gated drain sweep; pending UX + casing fix in `agent-status-selector`) ✅ SHIPPED 2026-06-06 — see [ADR-0009](docs/decisions/0009-agent-presence-session-work-continuity.md) + [W4 spec](docs/specs/2026-06-06-w4-deferred-pause.md). Session/Auth **W5 (digital work failover)** (auto-rescue of in-flight digital work when an agent goes Offline: `OfflineSince` grace + cancel-on-return, `queue_priority` front-jump re-queue with a 3-attempt anti-loop cap, leader-gated `WorkFailoverWorker`; supervisor stuck-work tab + reassign — voice caller-rescue **W5b deferred**) ✅ SHIPPED 2026-06-06 — see [ADR-0009](docs/decisions/0009-agent-presence-session-work-continuity.md) + [W5 spec](docs/specs/2026-06-06-w5-work-failover.md). Session/Auth **W5b (voice caller-rescue)** (when an agent's SIP leg dies mid-call the caller is dropped by the app_queue 2-party bridge → priority **callback** to the customer routed to the next live agent in the origin queue; detection via per-leg `HangupCause` on the existing `CallSession` + W3 liveness in a grace window; leader-gated `CallbackRescueWorker` + anti-loop + supervisor stuck-work voice + retry-callback) ✅ SHIPPED 2026-06-06 — PRs #45 (Platform) / #79 (Web) merged to main — see [ADR-0009](docs/decisions/0009-agent-presence-session-work-continuity.md) + [W5b spec](docs/specs/2026-06-06-w5b-voice-callback-rescue.md). Session/Auth **W6 (capacity configurability)** (per-tenant default channel capacity + sparse per-agent override resolved at read — seam `ICapacityDefaultsProvider`/`IAgentCapacityResolver` reusing the auth hot-path cache; `MaxTotal` now enforced async-only with voice an exclusive lane / `MaxVoice` pinned 1; chat-pool counter bug fixed; migration 033 with tenant-default columns + legacy `agents.capacity`→`'{}'` normalization; capacity-change audit; admin UI = per-agent override form + effective-capacity detail card + tenant defaults in operational settings, i18n 3 locales) ✅ SHIPPED 2026-06-07 — PRs #46 (Platform) / #80 (Web) merged to main — see [ADR-0009](docs/decisions/0009-agent-presence-session-work-continuity.md) + [W6 spec](docs/specs/2026-06-07-w6-agent-capacity.md). 🎉 **The ADR-0009 north-star is COMPLETE — W1+W2 · W3 · W4 · W5 · W5b · W6 all SHIPPED.**
