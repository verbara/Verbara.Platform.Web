...
const queueSchema = z.object({
  ...
  maxWaiting: z.coerce.number().int().positive().optional().or(z.literal('')),
  timezone: z.string().optional(),
  answerWithinSeconds: z.coerce.number().int().positive().optional().or(z.literal('')),
  firstResponseWithinSeconds: z.coerce.number().int().positive().optional().or(z.literal('')),
  resolutionWithinSeconds: z.coerce.number().int().positive().optional().or(z.literal('')),
  overflowQueueId: z.string().optional(),
  overflowAfterSeconds: z.coerce.number().int().positive().optional().or(z.literal('')),
  requiredSkills: z.array(z.object({ name: z.string().min(1) })),
  schedule: z.array(scheduleEntrySchema),
  defaultWrapUpSeconds: z.coerce.number().int().positive().default(30).or(z.literal('')),
  forceWrapUp: z.boolean().default(false),
...
export function QueueForm({ open, onOpenChange, mode, defaultValues, onSubmit }: QueueFormProps) {
  ...
  const defaults: QueueFormValues = {
    name: '',
    isActive: true,
    maxWaiting: '' as unknown as undefined,
    timezone: '',
    answerWithinSeconds: '' as unknown as undefined,
    firstResponseWithinSeconds: '' as unknown as undefined,
    resolutionWithinSeconds: '' as unknown as undefined,
    overflowQueueId: '',
    overflowAfterSeconds: '' as unknown as undefined,
    requiredSkills: [],
