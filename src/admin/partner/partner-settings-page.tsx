...
export default function PartnerSettingsPage() {
  ...
  if (isLoading) {
    return (
      <div className="flex h-64 items-center justify-center">
        <p className="text-sm text-muted-foreground">{t('partner.settings.loading')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6" data-testid="partner-settings-page">
      <PageHeader title={t('partner.settings.title')} description={t('partner.settings.description')}>
      ...
      <div className="max-w-2xl rounded-md border p-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label={t('partner.settings.platform_name')} value={settings?.platformName} testid="ps-platform-name" />
          <Field label={t('partner.settings.default_timezone')} value={settings?.defaultTimezone} testid="ps-timezone" />
          <Field label={t('partner.settings.default_language')} value={settings?.defaultLanguage} testid="ps-language" />
        </div>
        <p className="mt-4 text-xs text-muted-foreground">
          {t('partner.settings.managed_note')}
        </p>
      </div>
        ...
        <SheetContent side="right" className="sm:max-w-md">
          ...
          <div className="space-y-3 px-4">
            <div className="space-y-1.5">
              <Label htmlFor="ps-name-input">{t('partner.settings.edit_dialog.platform_name')}</Label>
              <Input
                id="ps-name-input"
            ...
            <div className="space-y-1.5">
              <Label htmlFor="ps-tz-input">{t('partner.settings.edit_dialog.default_timezone')}</Label>
              <Input
                id="ps-tz-input"
            ...
            <div className="space-y-1.5">
              <Label htmlFor="ps-lang-input">{t('partner.settings.edit_dialog.default_language')}</Label>
              <Input
                id="ps-lang-input"
          ...
          <SheetFooter className="px-4">
            <Button variant="outline" onClick={() => setEditOpen(false)}>{t('partner.settings.edit_dialog.cancel')}</Button>
            <Button onClick={handleSave} disabled={update.isPending} data-testid="ps-submit">
              {update.isPending ? t('partner.settings.edit_dialog.saving') : t('partner.settings.edit_dialog.save')}
            </Button>
          </SheetFooter>
        </SheetContent>
      </Sheet>
