...
- [ ] 1.1 Add `useForceOffline` mutation to `src/core/api/hooks/use-agents.ts`: input
...
- [ ] 1.2 Extend `AuthConfig` in `src/core/api/hooks/use-auth-admin.ts` with
...
- [ ] 2.1 Force-offline button on `src/admin/agents/agent-detail.tsx`: a destructive button
...
- [ ] 2.2 `pendingPauseTimeoutMinutes` `Input` on `src/admin/system/auth-config-page.tsx`: clone
...
- [ ] 3.1 Add the new keys to ALL THREE locales
...
- [ ] 3.2 `npm run build` — `tsc -b && vite build` must pass clean (drift-catching gate).
- [ ] 3.3 `npx vitest run` — add unit coverage: `useForceOffline` posts `{ revokeSessions }`
...
- [ ] 3.4 `npm run lint` + i18n parity: `npx eslint .` clean and `scripts/i18n-parity-check.mjs`
      (`i18n:check`) reports zero drift.
- [ ] 3.5 `npx playwright test` — E2E for the two flows using `data-testid` selectors only
