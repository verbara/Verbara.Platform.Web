...
**Affected**: the webchat embed bundle (Platform.Web) and the Platform CSAT capture endpoint
`POST /api/v1/csat/responses/webchat`. Because Web and Platform share this HTTP boundary, any
drift between this payload and the golden fixture breaks capture silently at runtime.

**API-contract dependency (API-first)**: this surface depends on Platform shipping
`POST /api/v1/csat/responses/webchat` accepting the `csat-response-capture.v1` shape and issuing
the `responseToken`. Web is a decoupled consumer (buildOrder 2); the contract leads.

**Mitigation**: pin the request DTO to the golden fixture field names (this spec), add a
contract/unit test that asserts the serialized body keys equal the fixture keys, and treat the
fixture as the single source of truth per the verbatim-fixture-citation rule.
