...
- [ ] 1.1 Add the typed CSAT capture request DTO mirroring the golden fixture field-for-field: `responseToken`, `surveyId`, `questionId`, `channel`, `queueName`, `rating`, `comment`, `capturedAt`, `conversationId` (single source of truth, no ad-hoc literals)
- [ ] 1.2 Add the TanStack Query mutation hook posting the DTO to `/api/v1/csat/responses/webchat`
- [ ] 1.3 Add i18n keys for all three surfaces (rating panel, KPI card, admin CSAT tab) across EN-US / ES-419 / PT-BR; keep parity green
- [ ] 1.4 Re-pin `package.json` 3.2.0-web → 3.13.0-web (baseline 3.12.0-web)
