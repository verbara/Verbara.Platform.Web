...
- [ ] 1.1 Confirm Platform's `openapi-numeric-schema-truth` host change has landed (its
...
- [ ] 1.2 Run `npm run generate:api-types` (`node scripts/generate-api-types.mjs`) to regenerate
      `src/core/api/generated/openapi.d.ts` from the corrected document.
- [ ] 1.3 Verify the union is extinct: `grep -c "number | string" src/core/api/generated/openapi.d.ts`
...
- [ ] 2.1 `use-analytics.ts` — alias the now-matching shapes (`DashboardKpis`/`DashboardData`,
...
- [ ] 2.2 `use-surveys.ts` — alias `Survey`, `SurveyQuestion`, `SurveySummary`, `SurveyAnswer`,
      `SurveyResponse` (and the `SurveyType`/`QuestionType` literal unions where the generated enum
      matches) to their generated schemas.
- [ ] 2.3 `use-recording.ts` — alias `RecordingMetadata` to its generated schema.
- [ ] 2.4 `use-csat.ts` — adopt the generated type for the CSAT capture flow where a clean match
...
- [ ] 2.5 `TopicTrendsResponse` (`use-analytics.ts`) — keep hand-written; the generated schema
      renames `topics`→`trends` and drops `from`/`to`. Log as a Platform contract bug.
- [ ] 2.6 `ComplianceRuleSummaryDto.severity` (`use-analytics.ts`) — keep hand-written; generated
      `severity` is widened from the `'Info' | 'Warning' | 'Critical'` literal union to `string`. Log
      as a Platform contract bug.
- [ ] 2.7 `PagedResult<T>` envelope (`use-analytics.ts`) — keep the local generic wrapper; generated
      is `{ data, hasMore }` vs the hook's `{ items, hasNextPage }`. Log as a Platform contract bug.
- [ ] 2.8 No-generated-counterpart shapes — keep hand-written: `BotAnalyticsSummary`,
      `TranscriptSegment`, `IntervalData` (all in `use-analytics.ts`), and request shapes like
      `CsatCaptureRequest` (`use-csat.ts`). No generated schema exists to adopt.
...
- [ ] 3.1 `use-billing.ts` — remove the `Number()` coercion sites (billing/invoice numeric fields).
- [ ] 3.2 `use-partner.ts` — remove the `Number()` coercion sites (partner numeric fields).
- [ ] 3.3 `use-queue-metrics.ts` — remove the `toQueueMetrics` mapper's `Number()` coercions over the
...
- [ ] 3.4 `use-analytics.ts` — remove the `Number()` coercions in the `CsatQueueSummary` /
      `CsatAggregateSummary` `select`-normalizers (`totalResponses`/`averageRating` now `number`).
- [ ] 3.5 `use-teams.ts` — remove the `memberCount` `Number()` coercion.
- [ ] 3.6 `use-notifications.ts` — remove the `count` `Number()` coercion.
- [ ] 3.7 `use-supervisor.ts` — remove the `failoverAttempts` `Number()` coercion (the
      `normalizeStuckConversation` wrapper).
- [ ] 3.8 `use-typification-llm.ts` — remove the `latencyMs` `Number()` coercion.
- [ ] 3.9 Update tests whose assertions covered the string arm — `use-csat-aggregate.test.ts`'s
...
- [ ] 4.1 Run `npm run lint:generated-types` (`node scripts/check-generated-types-adoption.mjs`) and
      let it force `generated-types-adoption-baseline.json` DOWN: remove `use-csat.ts`,
      `use-recording.ts`, and `use-surveys.ts` from `unadopted_hooks` as they adopt generated types
      (floor moves from 39). Never add to the list.
- [ ] 4.2 Correct `openspec/specs/openapi-generated-types/spec.md`'s "Numeric AOT wire unions get a
...
- [ ] 5.1 `npm run build` — `tsc -b && vite build` clean (the drift-catching CI gate; proves every
      retired coercion site and every adopted alias type-checks against the regenerated document).
- [ ] 5.2 `npx vitest run` — full unit suite green (with the obsolete string-arm assertions removed).
- [ ] 5.3 `npx eslint .` — clean, including `npm run i18n:check` and `npm run lint:generated-types`
      (ratchet floor lowered, no unadopted regression).
- [ ] 5.4 Final grep: `grep -c "number | string" src/core/api/generated/openapi.d.ts` is 0, and no
