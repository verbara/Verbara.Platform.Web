...
**Affected**: the Operations wallboard (`src/operations/wallboard/csat-kpi-card.tsx`,
`wallboard-page.tsx`), the API-hooks layer (`src/core/api/hooks/use-analytics.ts`), and the realtime
hub registration (`src/core/realtime/platform-hub.ts`) in Platform.Web.

**API-contract dependency (API-first)**: the card depends on the NEW Platform read endpoint
`GET /api/v1/analytics/csat` (host child `csat-completion` in `Verbara.Platform`, `buildOrder: 2`)
whose envelope is frozen by `fixtures/csat-aggregate-analytics.v1.json` (`totalResponses`,
`averageRating`, `rangeStart`, `rangeEnd`, `queues[]` rows of `queueName`, `channel`,
`totalResponses`, `averageRating`, `rangeStart`, `rangeEnd`; `channel` is `all` when unfiltered).
The realtime path depends on Pro's typed `IPlatformHubClient.OnCsatResponseRecorded`
(`buildOrder: 1`) whose payload is frozen by `fixtures/csat-response-recorded-payload.v1.json`
(`tenantId`, `responseId`, `surveyId`, `conversationId`, `channel`, `queueName`, `rating`,
`comment`, `capturedAt`). The generated `openapi.d.ts` aggregate schema lands only after Platform
ships; interim-fixture governance is `openapi-typed-client-phase2`, and the `useCsatAggregateAnalytics`
hook is the second concrete call site of that change's tracked `number | string` AOT-union
normalization pattern.

**Mitigation**: gate the card on the aggregate endpoint's availability and degrade to the existing
empty state when no data is returned; keep the SignalR handler as pure enrichment over the poll so
an unavailable realtime channel never leaves the score stale beyond one poll interval; cite the
frozen fixtures verbatim so the consumed wire keys cannot drift.
