...
test.describe('Cluster Management', () => {
  ...
  test('should display cluster management page', async ({ platformAdminPage: page }) => {
    await expect(page.getByTestId('cluster-page')).toBeVisible();
    await expect(page.getByTestId('cluster-add-node-btn')).toBeVisible();
  });

  test('should show summary cards', async ({ platformAdminPage: page }) => {
  ...
  test('should show nodes data table', async ({ platformAdminPage: page }) => {
    await expect(page.getByTestId('cluster-nodes-table')).toBeVisible();
  });

  test('should open add node sheet when clicking add button', async ({ platformAdminPage: page }) => {
    await page.getByTestId('cluster-add-node-btn').click();
    await expect(page.getByTestId('cluster-add-node-sheet')).toBeVisible();
  });

  test('should show instances section', async ({ platformAdminPage: page }) => {
