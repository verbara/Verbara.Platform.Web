...
test.describe('DNC Lists', () => {
  ...
  test('should display DNC lists page', async ({ platformAdminPage: page }) => {
    await expect(page.getByTestId('dnc-lists-page')).toBeVisible();
    await expect(page.getByTestId('dnc-lists-create-btn')).toBeVisible();
  });

  test('should create a DNC list', async ({ platformAdminPage: page, authenticatedApiContext }) => {
  ...
  test('should delete a DNC list with 3s confirmation', async ({ platformAdminPage: page, authenticatedApiContext }) => {
    const api = new ApiHelper(authenticatedApiContext);
