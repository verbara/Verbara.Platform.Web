...
- **On ship:** `git mv` this plan from `active/` → `completed/` when both PRs merge to `main`. **Next:** W5 (work failover — including the reassignment of the stuck work W4 marks on timeout), then W6 (capacity) — ADR-0009 north-star.
