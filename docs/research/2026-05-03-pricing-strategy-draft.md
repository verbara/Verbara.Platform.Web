...
- *"100% open-source UI and backend (Apache 2.0). Commercial license keys for the Pro engine. From $0 to enterprise — pay for what you actually use."*
- *"Vicidial-style transparency with Genesys-class operations. Without the per-minute Twilio bill."*

**Trust strip (logos when available):**
- "Built on `Asterisk.Sdk` (MIT) — see the source on GitHub"
...
**Get started:**
```bash
git clone https://github.com/Harol-Reina/Asterisk.Platform
docker compose up
```
...
A: Yes. `Asterisk.Sdk` is MIT-licensed. `Asterisk.Platform` (backend) and `Asterisk.Platform.Web` (UI) are Apache 2.0. You can inspect, modify, fork, and redistribute. The only commercial component is `Asterisk.Sdk.Pro`, which provides enterprise overlays (multi-tenant, advanced analytics, cluster, license gate). You only need it if you want those features — Community tier runs without it.
...
- Avoid claiming "open source" without qualifier — Pro is commercial. Always say *"open-source platform + commercial enterprise engine"* or similar.
